  ...
  if (props.fields.NavigationTitle) {
    text = (
      <span suppressHydrationWarning>
        <Text field={props.fields.NavigationTitle} />
      </span>
    );
...
export const Default = (props: NavigationProps): JSX.Element => {
  ...
  const handleToggleMenu = (event?: React.MouseEvent<HTMLElement>, flag?: boolean): void => {
    if (event && mode.isEditing) event.preventDefault();
    if (flag !== undefined) return openMenu(flag);
    openMenu(!isOpenMenu);
  };
  ...
  return (
    <div className={`component navigation ${styles}`} id={id ? id : undefined}>
      {/* class/children can change when menu opens → suppress mismatch */}
      <label className="menu-mobile-navigate-wrapper" suppressHydrationWarning>
        <input
          type="checkbox"
  ...
  return (
    <section className="py-16">
      <div className="container mx-auto">
        ...
        {/* items are authored; wrap to avoid small SSR/CSR diffs */}
...
const NavigationList = (props: NavigationProps) => {
  ...
  return (
    // active class can change client-side → suppress mismatch on the li
    <li className={`${classNameList} ${active ? 'active' : ''}`} key={props.fields.Id} tabIndex={0} suppressHydrationWarning>
      <div
