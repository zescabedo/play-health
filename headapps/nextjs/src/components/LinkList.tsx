...
const LinkListItem = (props: LinkListItemProps) => {
  ...
  if (props.index == 0) className += ' first';
  if (props.index + 1 == props.total) className += ' last';
    ...
    return (
      <div data-class-change className={styles} id={id ? id : undefined}>
        <div className="flex flex-col">
          {/* Wrap user text to avoid SSR/CSR hiccups */}
  ...
  if (datasource) {
    ...
    return (
      <div
        ...
        id={id ? id : undefined}
      >
        {/* Client toggles active class → avoid hydration mismatch */}
        <div className="container mx-auto px-4" suppressHydrationWarning>
          <ul className="flex gap-12 list-none p-0 m-0" role="listbox" aria-label="Navigation options">
            {list}
          </ul>
        </div>
      </div>
  ...
  if (datasource) {
    const list = datasource.children.results
      ...
      .map((element: ResultsFieldLink, key: number) => {
        ...
        return (
          <React.Fragment key={`${key}${href}-frag`}>
            <ContentSdkLink
              key={`${key}${href}-link`}
    ...
    return (
      <div className={` ${styles}`} id={id ? id : undefined} data-class-change>
        <div className={`flex flex-col lg:flex-row ${parentBasedStyles} gap-x-10 gap-y-6 font-[inherit]`}>
          {list}
        </div>
      </div>
    );
    ...
    return (
      <div className={`flex flex-col gap-2 ${styles}`} id={id ? id : undefined} data-class-change>
        <h5 className="text-sm font-(family-name:--font-accent) font-medium text-secondary-foreground uppercase">
          {/* Title is authored text → guard it */}
